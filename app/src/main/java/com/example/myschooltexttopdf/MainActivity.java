package com.example.myschooltexttopdf;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void createPdf() throws FileNotFoundException,DocumentException{
       ...
       pdfFile = new File(docsFolder.getAbsolutePath(),"PDFgenerate.pdf");
        ...
        PdfWriter.getInstance(docs,output);
        docs.open();
        docs.add(new Paragraph(mContentEditText.getText().toString()));
        docs.close();
    ...
    private void previewPdf(){
        ...
        Intent testItent = new Intent(Intent.ACTION_VIEW);

        testItent.setType("application/pdf");
        List list = packageManager.queryIntentActivities(testItent,PackageManager.MATCH_DEFAULT_ONLY);
        if (list.size() > 0) {
            Intent intent = new Intent();
            intent.setAction(Intent.ACTION_VIEW);
            Uri uri = FileProvider.getUriForFile(this,"com.example.myschooltexttopdf.fileprovider",pdfFile);
